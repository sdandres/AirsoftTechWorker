from tkinter import TclError, ttk, Canvas, Entry, Button, PhotoImage, VERTICAL, Scrollbar, Label, Toplevel, END, StringVar, messagebox
...
def open_inventory(window):
    ...
    window.inventory_button_image = PhotoImage(
        file=relative_to_assets("empty_button.png"))
        ...
        if selected_item:
            ...
            for ticket in tickets:
                if ticket["LaborID"] == item_values[0]:
                    ts.open_ticket_editor(ticket, refresh_callback=refresh_tickets)
                    break

    def refresh_tickets():
        nonlocal tickets
        try:
            if not str(tree):  # Tree no longer exists
                return
            tickets = ts.read_ticket_data_from_access()
...
def open_login_page(window):
    window.bind_all("<Tab>", lambda e: (e.widget.tk_focusNext().focus(), "break")[1])
    window.canvas = Canvas(
        window,
        bg = "#FFFFFF",
        height = 720,
